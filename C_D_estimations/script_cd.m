...
f_b = 7.105; % fineness ratio of the body
A_W_B = 0.6212; % total wetted area of the body
A_W_N = 0.085; % nosecone wetted area of the 
S = 0.1; % fin's span
gamma_L = pi/4; % leading edge sweepback angle
r_L = 0.0002; % radius of the rounded leading edge
c_r = 0.2; % fin's root chord
h_r = 0.0004; % root trailing edge thickness of the fin
t_r = 0.005; % maximum thickness of the fin
...
if Re <= 5e5
    ...
    rough_C_f = 0;
elseif Re <= Re_cr
    tran_C_f = (1 / (3.46 * log10(Re) - 5.6)^2) - (1700 / Re)
    lam_C_f = 0;
    rough_C_f = 0;
else
...
if Re_M < 5e5
    C_f_C_M = 1.328 / sqrt(Re_M);
elseif Re_M < Re_cr
    C_f_M = 1 / (3.46 * log10(Re_M) - 5.6)^2 - 1700 / Re_M;
...
if mach^2 <= 0.1 %incompressible flow
    ...
    C_D_f_b = (1 + (0.5 / f_b)) * C_f * A_W_N / A_r; % body
    ...
    K = cos(gamma_C)^2 + ((C_D_TT_M * A_r / (C_f_C_M * A_T) - ...
        ...
        (120 * (t_r / c_r)^4 * cos(gamma_C)^2))^(2/3);% prandtl correcting factor
    ...
    K_og = 1 + (6.82 * A_W_B * C_f_C_M * (f_n + 1)^2.22 / (f_b^3 * A_r))^(5/3);
    % ogive
    C_D_P = 6 * A_W_B * C_f / (f_b^3 * A_r * (K_og - mach^2)^0.6);% body
    % pressure drag
    C_D_B = 0.029 * sqrt(A_B) / sqrt(C_f * A_W_B); % body base drag
    %% TOTAL DRAG COEFFICIENT
    C_D = C_D_f + C_D_L_T + C_D_B_T + abs(C_D_TT) + C_D_P + C_D_B;
elseif mach <= 1 %subsonic flow
    ...
    C_D_f_b = (1 + (0.5/f_b)) * C_f_C * A_W_N / A_r; % body
    C_D_f = C_D_f_b + C_D_f_t; % friction drag coefficient
    %% FINS' DRAG COEFFICIENTS
    if mach < 0.9
        delta_C_D = ((1 - mach^2)^-0.417) - 1;
