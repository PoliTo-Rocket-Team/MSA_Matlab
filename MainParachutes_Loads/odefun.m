...
dydt(2)=(3/2).*rho.*k_a.*(y(4).^(1/2)).*(dydt(4));
dydt(1)=-(y(3)+(0.5.*rho.*y(1).^2.*(CdS_r+CdS_d))-W_V)./m_V;
dydt(3) = 0.5.*rho.*(2.*y(1).*dydt(1).*y(4)+((y(1)).^2).*dydt(4))+dydt(1).*dydt(2)+dydt(2).*dydt(1)-(1/m_V).*(y(2)+m_p).*(dydt(3)+rho.*y(1).*dydt(1).*(CdS_r+CdS_d))+0.75.*rho.*k_a.*((dydt(4)).^2).*y(1)./(sqrt(y(4)));
end
