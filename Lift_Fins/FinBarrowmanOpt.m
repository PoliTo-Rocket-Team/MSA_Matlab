...
% This variable is used to set which program you should use
% 1 = optimization with set fin semispan 's'
% 2 = optimization with set root chord length 'c_r'
...
if program == 1
    ...
    s = 100;
    sweep = 45 * (pi/180); % Leave pi/180 as written, just change the sweep angle, it will convert it in radiant.
    
    % Input lower bond and upper bond here, they will be Root Chord
    % Boundaries
    lb = 0; % Lower
    ub = 200; % Upper. ub must be > s due to geometrial limits. If ub = s, the shape of the fin is a triangle.
    ...
    if s > 0 && sweep > 0 && n > 0 && lb >= 0 && (ub > 0 && ub > s)
        ...
        if(ub <= s)
            ...
            disp('Variables [s, n, lb, ub] must be positive. Code cannot perform.');
        end
    end
...
elseif program == 2
    ...
    c_r = 300;
    sweep = 45 * (pi/180); % Leave pi/180 as written, just change the sweep angle, it will convert it in radiant. I know this is redundant, but in order to have a cleaner code I wrote it twice

    lb = 0;
    ub = 100; % Due to geometrical limits, ub must be < c_r. If it's the same the shape of the fin is a triangle.
    n = 300;
    ...
    if c_r > 0 && sweep > 0 && n > 0 && lb >= 0 && (ub > 0 && ub < c_r)
        ...
        if(ub >= c_r)
            ...
            disp('Variables [c_r, lb, ub, n] must be positive. Code cannot perform.')
        end
    end
else
